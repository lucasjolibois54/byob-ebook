      ...
      return (
        <div className={`overflow-hidden bg-[#131313] block md:hidden z-40 text-white ${mobileNavOpen || isAnimating ? 'h-screen' : 'h-auto'}`}>
          ...
          >
            ...
            <motion.div variants={mobileMenuVariant} className={`fixed top-0 left-0 h-screen w-full flex flex-col items-center bg-black z-40`}
            ...
            >
              ...
              <motion.ul variants={ulVariant} className="list-none mt-[40px]">
                {MOBILE_NAV_ITEMS.map(navItem => (
                  <motion.li whileTap={{ scale: 0.95 }} key={navItem.id} className="my-[20px] overflow-y-hidden select-none">
                    <motion.div variants={liVariant} className="text-center capitalize text-[34px] hover:italic hover:cursor-pointer">{navItem.navTitle}</motion.div>
                  </motion.li>
                ))}
              </motion.ul>
